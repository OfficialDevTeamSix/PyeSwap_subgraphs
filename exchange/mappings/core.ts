...
import { updatePairDayData, updateTokenDayData, updatePyeDayData, updatePairHourData } from "./dayUpdates";
...
export function handleSync(event: Sync): void {
  ...
  pye.totalLiquidityBNB = pye.totalLiquidityBNB.minus(pair.trackedReserveBNB as BigDecimal);
  ...
  pye.totalLiquidityBNB = pye.totalLiquidityBNB.plus(trackedLiquidityBNB);
  pye.totalLiquidityUSD = pye.totalLiquidityBNB.times(bundle.bnbPrice);
...
export function handleMint(event: Mint): void {
  ...
  pye.totalTransactions = pye.totalTransactions.plus(ONE_BI);
...
export function handleSwap(event: Swap): void {
  ...
  let pye = PyeFactory.load(FACTORY_ADDRESS);
  pye.totalVolumeUSD = pye.totalVolumeUSD.plus(trackedAmountUSD);
  pye.totalVolumeBNB = pye.totalVolumeBNB.plus(trackedAmountBNB);
  pye.untrackedVolumeUSD = pye.untrackedVolumeUSD.plus(derivedAmountUSD);
  pye.totalTransactions = pye.totalTransactions.plus(ONE_BI);
  ...
  let pyeDayData = updatePyeDayData(event);
  let token0DayData = updateTokenDayData(token0 as Token, event);
  let token1DayData = updateTokenDayData(token1 as Token, event);

  // swap specific updating
  pyeDayData.dailyVolumeUSD = pyeDayData.dailyVolumeUSD.plus(trackedAmountUSD);
  pyeDayData.dailyVolumeBNB = pyeDayData.dailyVolumeBNB.plus(trackedAmountBNB);
  pyeDayData.dailyVolumeUntracked = pyeDayData.dailyVolumeUntracked.plus(derivedAmountUSD);
  pyeDayData.save();
